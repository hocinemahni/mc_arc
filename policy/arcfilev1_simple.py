...
class ARC_File_Policyv1simpl(Policy):
    ...
    def evict(self):
        ...
        self.evicted_blocks_count += worse_file.size +1
        self.evicted_file_count += 1

        # Calculer la taille des données à transférer en octets
        data_size_to_transfer = ((worse_file.size +1) * self.block_size)
        ...
        if len(self.t1) + len(self.b1) == self.c:
            if len(self.t1) < self.c:
                for j in range(worse_file.size +1):
                    self.b1.popitem()
    ...
    def on_io(self, file, timestamp, offset, io_size):
        ...
        for block_offset in range(offset, offset + io_size):
            ...
            # If a block is in T2 and is accessed, we move this single block to the top of T2
                ...
                #self.move_file_to(block[0], self.t2)
                #self.move_file_to(block[0], self.t2)
                blocks = self.file2blocks[file]
                for bloc in blocks:
                    del self.t2[bloc]
        ...
        print('les éléments de t1:')
        # for file, _ in self.t1.keys().mapping:
        #     print(file.name)
        #
        # # De même pour self.t2
        # print('les éléments de t2:')
        # for file, _ in self.t2.keys().mapping:
        #     print(file.name)
        # print('les éléments de b1:')
        # for file, _ in self.b1.keys().mapping:
        #     print(file.name)
        #
        # # De même pour self.t2
        # print('les éléments de b2:')
        # for file, _ in self.b2.keys().mapping:
        #     print(file.name)