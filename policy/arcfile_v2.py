...
class ARC_File_Policyv2(Policy):
    ...
    def evict(self):
        ...
        ########print(f'migrate time for this file , {worse_file} , avec la tail {worse_file.size}est  {migration_time}')
        ...
        for block_offset in range(offset, offset+io_size):
            ...
            # If an evicted file's block is in B1 and is accessed, we move all its blocks to T2
                ...
                if file.size < self.c - (len(self.t1) + len(self.t2)):
                 ...
                 self.ssd_tier.add( file)
                 self.hits_in_hdd_b1_b2 += 1 
                 self.prefetch_times += ((file.size * 4096) / self.ssd_tier.read_throughput) + ((file.size * 4096)/ self.hdd_tier.write_throughput) + self.ssd_tier.latency + self.hdd_tier.latenc
    ...
    """def __repr__(self) -> str:
        ...
        log_filename = os.path.join(logs_dir, f'arcv2_{current_datetime}.txt')
        ...
        #logging.info(output)
        #logging.debug('C\'est le nombre de hite v2: %s', self.hits)
        #logging.debug('cest le temps de migration v2 %s', sum(self.migration_times) )
        return output"""
